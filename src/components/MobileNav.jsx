...
export default function MobileNav({ count = 0, isLoggedIn, handleLogout }) {
  ...
  const HIDE_NAV_ROUTES = ["/autoSuggest"];
  ...
  if (HIDE_NAV_ROUTES.includes(location.pathname)) {
    return null;
  }
  ...
  return (
    <div className="mobile-nav-container">
      ...
      <div
        className="mobile-nav-back header-title"
        style={location.pathname === "/wishlist" ? { top: "11px" } : {}}
      >
        {location.pathname === "/" ? (
          <div className="mobile-logo-wrapper">
            <img src={Logo} alt="NovaCart" className="mobile-nav-logo" />
          </div>
        ) : (
          <>
            ...
            <br />
            {showCount && (
              <span className="header-sub-heading text-ellipse">
                {count} items
              </span>
            )}
          </>
      ...
      <div className="header-actions pull-right">
        ...
        {!hideIcons && (
          <div className="header-icon" onClick={() => requireAuth("/cart")}>
            <svg viewBox="0 0 24 24">
              <path
                fill="#3E4152"
                fillRule="evenodd"
            ...
            {isLoggedIn && cartCount > 0 && (
              <span key={cartCount} className="bag-badge animated pulse">
                {cartCount}
              </span>
            )}
          </div>
